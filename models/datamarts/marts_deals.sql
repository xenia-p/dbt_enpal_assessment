{{ config(
    materialized='table',
    post_hook=[
      "create index if not exists idx_marts_deals_deal_id on {{ this }} (deal_id)", 
      "create index if not exists idx_marts_deals_user_id on {{ this }} (last_user_id)"
    ]
) }}
...
FROM
  ...
  FROM {{ ref('interm_deals_times_users') }}
